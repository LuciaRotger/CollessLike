#' @title Generates random trees and computes their balance indices
#' 
...
#'
#' @details Given a number of leaves, the function generates a tree with that number of leaves and computates the three index of balance (Colles-like, Sackin and Cophenetic with function \code{\link{balance.indices}}). This is done as many times as it is set by 'repetitions' parameter , and it generates a 3-column data.frame of indices. 
#' 
#' The trees are generated according to the alpha-gamma model. This parameters can be specified by \code{alpha} and \code{gamma} parameters of the function. The following cases are distinguish:
#' \itemize{
...
#' } 
#'
#' @return A 3-column data.frame with the indices of Colless-like, Sackin and Cophenetic for every generated tree. If more than one data.frame has been generated, then the returned value is a data.frame list (its names specify which alpha and gamma parameters have generated that data.frame, for instance "a0.5g0.3" indicates alpha=0.5 and gamma=0.3). 
#' 
...
#' 
#' @examples #('Repetitions' set as 10 for a fast example)
